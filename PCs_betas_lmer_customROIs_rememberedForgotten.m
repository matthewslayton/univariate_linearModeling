...
allMasksArr = table2array(allMasks); %subjNum, itemID, 102 masks for 104 total cols
...
%         save('IDs_330.mat','IDs_330')
        ...
        %%%%%%% this doesn't work!! I'm just adding 0s. There aren't any actual subject numbers here.
        %%%%%%% allMasksArr has 300 rows per subject. That's what I need to grab and then remove the NaNs from
        %%%%% right now I'm just filling subjects_numbers_remembered with 0s

        %subject_numbers = allMasksArr(:,1);
        subject_numbers_remembered_300 = allMasksArr(subjectCounter:subjectCounter+299,1);
        subject_numbers_forgotten_300 = allMasksArr(subjectCounter:subjectCounter+299,1);
        %%% Step 1: Grab the subject-specific rows 
        %subjectSpecificData = maskArray_ID_data(subjectCounter:subjectCounter+299,:);
        subjectSpecificData_remembered_300 = maskArray_ID_data(subjectCounter:subjectCounter+299,:);
        subjectSpecificData_forgotten_300 = maskArray_ID_data(subjectCounter:subjectCounter+299,:);
        ...
        subject_numbers_remembered_subset = subject_numbers_remembered_300(~isnan(mem_old));
        subject_numbers_forgotten_subset = subject_numbers_forgotten_300(~isnan(mem_old));
        subjectSpecificData_remembered_subset = subjectSpecificData_remembered_300(~isnan(mem_old),:);
        subjectSpecificData_forgotten_subset = subjectSpecificData_forgotten_300(~isnan(mem_old),:);
        % ^ the "subset" naming convention fits with the others. I have
        % real values for factors, lexMem, etc, and I need to split them by
        % remembered or forgotten

        % 300 trials, and then for S002 we get down to 266. That's because
        % TT4 means there was a mismatch in covert naming (a false alarm).
        % So, we cut down to TT3 only which is some number less than 300.
        % Combined those are the remembered and forgotten together, and we
        % have to split them. mem_old_short has the remembered and
        % forgotten together and 1s and 0s to tell us which is which


        % zeros aren't a good idea. My factors have 0 rows. I need to do
        % NaNs instead
       % score_remembered = zeros(length(mem_old_short),994);
       % score_forgotten = zeros(length(mem_old_short),994);
        factor_remembered = NaN(length(mem_old_short),size(F,2));
        factor_forgotten = NaN(length(mem_old_short),size(F,2));
        subjectSpecificData_remembered = NaN(length(mem_old_short),numMasks+1); 
        subjectSpecificData_forgotten = NaN(length(mem_old_short),numMasks+1); 
        lexMem_remembered = NaN(length(mem_old_short),1);
        lexMem_forgotten = NaN(length(mem_old_short),1);
        visMem_remembered = NaN(length(mem_old_short),1);
        visMem_forgotten = NaN(length(mem_old_short),1);
        subject_numbers_remembered = NaN(length(mem_old_short),1);
        subject_numbers_forgotten = NaN(length(mem_old_short),1);
        ...
        for row = 1:numel(mem_old_short)
            if mem_old_short(row) == 1
                ...
                subjectSpecificData_remembered(row,:) = subjectSpecificData_remembered_subset(row,:);
                lexMem_remembered(row) = lexMem_subset(row);
                visMem_remembered(row) = visMem_subset(row);
                subject_numbers_remembered(row) = subject_numbers_remembered_subset(row);
            elseif mem_old_short(row) == 0       
               % score_forgotten(row,:) = score_subset(row,:);
                factor_forgotten(row,:) = factor_subset(row,:);
                subjectSpecificData_forgotten(row,:) = subjectSpecificData_forgotten_subset(row,:);
                lexMem_forgotten(row) = lexMem_subset(row);
                visMem_forgotten(row) = visMem_subset(row);
                subject_numbers_forgotten(row) = subject_numbers_forgotten_subset(row);
            end
        end
    
        % don't remove zero rows. factors have zero rows. remove NaNs
...
%        end
        ...
        if currFac == 1 || currFac == 2 || currFac == 3 || currFac == 4
            factor_remembered = factor_remembered(any(~isnan(factor_remembered),2),:);
            factor_forgotten = factor_forgotten(any(~isnan(factor_forgotten),2),:);
            subjectSpecificData_remembered = subjectSpecificData_remembered(any(~isnan(subjectSpecificData_remembered),2),:);
            subjectSpecificData_forgotten = subjectSpecificData_forgotten(any(~isnan(subjectSpecificData_forgotten),2),:);
            lexMem_remembered = lexMem_remembered(any(~isnan(lexMem_remembered),2),:);
            lexMem_forgotten = lexMem_forgotten(any(~isnan(lexMem_forgotten),2),:);
            visMem_remembered = visMem_remembered(any(~isnan(visMem_remembered),2),:);
            visMem_forgotten = visMem_forgotten(any(~isnan(visMem_forgotten),2),:);
            subject_numbers_remembered = subject_numbers_remembered(any(~isnan(subject_numbers_remembered),2),:);
            subject_numbers_forgotten = subject_numbers_forgotten(any(~isnan(subject_numbers_forgotten),2),:);
        %elseif currFac == 5 || currFac == 6 %the numbers just aren't lining up. Try this
            %subjectSpecificData_remembered(all(~isnan(subjectSpecificData_remembered),2),:) = [];
            %subjectSpecificData_forgotten(all(~isnan(subjectSpecificData_forgotten),2),:) = [];
%             subjectSpecificData_remembered = subjectSpecificData_remembered(any(~isnan(subjectSpecificData_remembered),2),:);
%             subjectSpecificData_forgotten = subjectSpecificData_forgotten(any(~isnan(subjectSpecificData_forgotten),2),:);
%             factor_remembered(all(~isnan(factor_remembered),2),:) = [];
%             factor_forgotten(all(~isnan(factor_forgotten),2),:) = [];
%             lexMem_remembered(all(~isnan(lexMem_remembered),2),:) = [];
%             lexMem_forgotten(all(~isnan(lexMem_forgotten),2),:) = [];  
%             visMem_remembered(all(~isnan(visMem_remembered),2),:) = [];
%             visMem_forgotten(all(~isnan(visMem_forgotten),2),:) = [];  
%             subject_numbers_remembered(all(~isnan(subject_numbers_remembered),2),:) = [];
%             subject_numbers_forgotten(all(~isnan(subject_numbers_forgotten),2),:) = [];
        end
...
%     end %end currFac if statement for making a struct to hold the F values
    ...
    %%% before I add oneROI_visMem and lexMem, replace the 999's with NA

    % Replace 999 with NaN
    oneROI_visMem(oneROI_visMem == 999) = NaN;
    oneROI_lexMem(oneROI_lexMem == 999) = NaN;
...
%     end %end currFac if statement
    ...
    if currFac == 1
        currSheet = 'avgActivity_remembered_BNA_nnmf_encycl_additionalROIs_unilateral.xlsx';
    elseif currFac == 2
        currSheet = 'avgActivity_remembered_BNA_nnmf_encycl_300_additionalROIs_unilateral.xlsx';
    elseif currFac == 3
        currSheet = 'avgActivity_remembered_BNA_nnmf_vis_additionalROIs_unilateral.xlsx';
    elseif currFac == 4
        currSheet = 'avgActivity_remembered_BNA_nnmf_vis_300_additionalROIs_unilateral.xlsx';
    elseif currFac == 5
...
%     end %end currFac if statement
    ...
    if currFac == 1
        currSheet = 'avgActivity_forgotten_BNA_nnmf_encycl_additionalROIs_unilateral.xlsx';
    elseif currFac == 2
        currSheet = 'avgActivity_forgotten_BNA_nnmf_encycl_300_additionalROIs_unilateral.xlsx';
    elseif currFac == 3
        currSheet = 'avgActivity_forgotten_BNA_nnmf_vis_additionalROIs_unilateral.xlsx';
    elseif currFac == 4
        currSheet = 'avgActivity_forgotten_BNA_nnmf_vis_300_additionalROIs_unilateral.xlsx';
    elseif currFac == 5
...
%%%%% gotta be honest. I don't really understand what this is here. It looks like 
%%%%% a replication of something I did above
...
% 
% %%%%% scatter of mem x F
% below here now lives on its own in PCs_betas_etc_scatter_mem_F.m
